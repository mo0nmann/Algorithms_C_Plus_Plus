...
namespace pluto {
    ...
    namespace sorting {
        ...
        void librarySort(std::vector<int> *vec, int epsilon) {
            ...
            std::vector<int> S = __detail::createGappedVector(vec, epsilon);
            ...
            for (int i = 0; i < floor(log2(n) + 1); i++) {
                ...
                for (int j = a; j < std::min(b, n + 1); j++) {
                    ...
                    int insert_pos = __detail::binarySearch(element_to_sort, &S, next_index);
                ...
                // rebalance
                __detail::rebalance(&S, 0, next_index, epsilon);  
